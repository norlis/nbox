...
func (r *SpecRegistry) Validate(ctx context.Context, specID string, data []byte, format string) (Result, error) {
	spec, err := r.GetByID(specID)
	if err != nil {
		return NewResult(WithInternalError(err)), nil
	}
...
func (r *SpecRegistry) ValidateByFilename(ctx context.Context, filename string, data []byte, format string) (Result, error) {
	spec, found := r.Resolve(filename)
	if !found {
